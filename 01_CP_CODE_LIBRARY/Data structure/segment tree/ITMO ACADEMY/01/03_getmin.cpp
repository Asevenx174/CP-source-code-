...
struct segtree
{
   ...
   void sett(int idx,int val,int x,int lx,int rx)
   {
      if(rx-lx==1)
      {
         mins[x] = val ; return ;//*** node (x) a val set hbe
      }
   ...
   {
       //cout << lx << " " << rx << endl ;
       if(r<=lx || rx<=l) { return mod ; }
       if(l<=lx && rx<=r) { return mins[x] ; } 
       int mid = (lx+rx)/2 ;   //** mid is middle of lx,rx 
       int lc = 2*x+1 , rc = 2*x+2 ;  
       ll s1 = getmin(l,r,lc,lx,mid) ;
       ll s2 = getmin(l,r,rc,mid,rx) ;
       return min(s1,s2) ; 
   }
...
int main()
{
   ...
   while(m--)
   {
      ...
      else
      {
         int l,r ;
         cin >> l >> r ; 
         cout << st.getmin(l,r) << endl ; // find sum of [l..r-1] range 
      }
   }
   return 0 ;
}
...
struct segtree
{
   ...
   int first_above(int v,int x,int lx,int rx)
   {
      ...
      int mid = (lx+rx)/2 ;   //** mid is middle of lx,rx 
      int lc = 2*x+1 , rc = 2*x+2 ;
      int res = first_above(v,lc,lx,mid) ;
      if(res==-1)
      {
         res = first_above(v-values[lc],rc,mid,rx) ; 
      }
      return res ; 
   }
   int first_above(int v)
   {
      return first_above(v,0,0,siz);
   }
};
int main()
{
   ...
   segtree st ; 
   st.init(n) ; 
   std::vector< int > ar(n+5,0),pr(n+5),vis(n+5,1),ans(n+5) ;
   for (int i = 0; i < n ; ++i)
   {
      cin >> ar[i] ; 
   }
   st.build(vis) ; // instead of call n times sett fun ,here we call once build fun o(siz)
   int all = n ; 
   for (int i = n-1 ; i>=0 ; i-- )
   {
      int idx = st.first_above(all-ar[i]-1) ;
      ans[i] = idx+1 ; 
      st.sett(idx,0) ; 
      all-- ; 
   }
